...
`PREVIOUS_IMAGE_TAG` из `.env`, **возвращает compose-файлы того же коммита**, тянет его
образы, пересоздаёт `api` и `edge` и проверяет здоровье ещё раз.

Compose откатывается вместе с образами не для красоты. 19.08.2026 переезд фотографий
на диск убрал из compose переменные `S3_*`; выкатка упала, откат вернул образ, который
их требует, — и стенд ушёл в цикл перезапусков с пустым `{}` в логе. Выглядело как
«откат тоже не поднялся», хотя и код, и конфиг по отдельности были рабочими. `.env` при провале не переписывается: `IMAGE_TAG` остаётся указывать на
работающую версию.
