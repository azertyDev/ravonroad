...
2. удаляет все образы кампании, кроме двух: выкатываемого и развёрнутого сейчас — он же
   цель отката. Образы тегируются `sha`, поэтому каждая выкатка приносит три новых
   и ни одного не убирает: на стенде за день набралось 65 образов и 25 ГБ, диск кончился,
   Postgres упал с `No space left on device` и ушёл в цикл восстановления. Сайт лёг
   не от нагрузки и не от кода, а от собственных выкаток.

   Раньше здесь стоял порог по возрасту, `until=72h`. Он не работает: 19.08.2026 за день
   набежало 62 образа и 24 ГБ, диск дошёл до 98%, и **не удалилось ничего** — все образы
   были моложе порога. Время защищает от редких выкаток и бесполезно ровно в тот день,
   когда выкаток много. Больше двух версий на машине не нужно никогда: остальное вернёт
   `docker pull` из GHCR, просто медленнее;
