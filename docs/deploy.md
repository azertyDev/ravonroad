...
2. тянет образы **по одному** (`migrate`, `api`, `edge`): параллельная распаковка трёх
   слоёв не помещается в свободные ~80 МБ (SRS §12.2);
...
| | dev | prod |
| --- | --- | --- |
| Память | 1 ГБ, лимиты 672 МБ (SRS §12.2) | 2 ГБ, лимиты 1312 МБ (SRS §12.1) |
