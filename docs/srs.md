...
| Том `photos` | Оригиналы (≤1600 px) и превью (400 px) на диске сервера. Пишет `api`, раздаёт `edge` (ADR-0009) | внутри периметра |
| S3-совместимое хранилище | Только резервные копии: дампы БД и ночная копия фотографий (§10.5). Внешний сервис | вне периметра |
| Telegram Bot API | доставка карточек, приём callback и фото «после» | вне периметра |
| Подложка карты | MapLibre GL + экстракт Ташкента в PMTiles. Один статический файл в образе edge, браузер читает его range-запросами. Геокодера нет (ADR-0004, ADR-0008) | внутри периметра: наш же origin, как и фотографии (ADR-0009) |
...
```
 ...
 └────┬──────────────────────────────┬─────────────────────┘
      │ SQL (Prisma + $queryRaw)     │ файловые операции
      v                              v
 ┌──────────────────────┐     ┌─────────────────┐
 │ db: Postgres+PostGIS │     │ том photos      │
 │ единственное         │     │ фото + превью,  │
 │ состояние системы    │     │ раздаёт edge    │
 └──────────────────────┘     └─────────────────┘
```
...
сервер не должен считать кластеры на каждый жест зума, а карта делает это
...
**Почему таблица Postgres, а не Redis или BullMQ.** Очередь живёт в бюджете контейнеров
1312 МБ (§12.1), где Redis стоил бы ~40 МБ резидентно плюс четвёртый постоянный контейнер,
а BullMQ — ещё и отдельный процесс.
...
[требует проверки: фактическая задержка Cloupard S3 из сети прод-машины; при 200 мс на операцию
пропускная способность падает до ~1,2 фото/с и запас исчезает — тогда воркеров становится
два, это одна константа]

### 5.5 Ключи на диске и дедупликация
...
Ключи стали путями относительно `MEDIA_ROOT` — фотографии лежат на диске сервера
(ADR-0009). Схема не изменилась: контент-адресация переносится на файловую систему
как есть.
...
Обычная жизнь файла — секунды: воркер удаляет его сам после успешной обработки.
Забытое подметает **сборщик в `PhotoWorker`: раз в час, файлы старше суток**. Раньше это
делало lifecycle-правило бакета одной строкой конфигурации; вместе с бакетом правило
исчезло, и сборщик пришлось написать. Сутки, а не час: при пяти попытках с backoff
последняя приходится почти на час, и подмести файл, за которым воркер ещё вернётся,
значит превратить восстановимую ошибку в потерянную фотографию.
...
- **Двухсимвольный префикс** разбрасывает файлы по 256 каталогам. Придуман ради вменяемого
  листинга бакета и оказался ровно тем приёмом, каким на диске избегают каталога
...
- **Осиротевшие файлы.** Если транзакция §5.1.13 упала после записи, файл остаётся
  в `incoming/` без строки в БД. Их и подметает сборщик выше — на диске сервера цена
  такой сироты выше, чем в бакете: место кончается раньше памяти и кончается тихо.
- **Запись атомарна**: временный файл в `.tmp` и `rename`. Прямая запись оставила бы
  при падении процесса обрезанный JPEG под ключом, который по построению неизменяем, —
  браузер закэшировал бы битый файл на год.
- Заголовки `Content-Type: image/jpeg`, `Cache-Control: public, max-age=31536000, immutable`
  и `Content-Disposition: inline` выставляет edge: на диске хранить их негде, и это
  единственное, что переехало из свойств объекта в конфигурацию nginx.
- Публичное чтение — `/media/photos/…` с нашего же origin; путь `/media/` целиком отвечает
  `404`, потому что на том же томе лежит `incoming/`. Запись — только из api; том у edge
  смонтирован на чтение. Клиент не пишет в хранилище никогда: валидация и вырезание EXIF
  обязаны стоять между пользователем и файлом.
...
### 6.3 Карточка: одно сообщение или два

| # | Сообщение | Содержимое |
| --- | --- | --- |
| 1 | `sendMediaGroup` (2–3 фото) | Подпись на первом фото: номер `RR-3471` ссылкой на `/uz/reports/3471`, район, категория, ориентир, дата/время, ссылка на точку в Яндекс Картах, строка «⚠️ tekshirish: …» при сигналах антиабуза (US-025), строка «yaqin atrofda: RR-3412, RR-3448» при найденных дублях (§3.2) |
| 2 | `sendMessage` ответом на #1 | Строка статуса + inline-клавиатура. Это сообщение и редактируется при каждом переходе |

**Одна фотография — одно сообщение.** `sendPhoto` принимает и подпись, и `reply_markup`,
поэтому у заявки с единственным фото карточка не делится: подпись, состояние и кнопки
уходят вместе, группа получает одно уведомление вместо двух, а бюджет канала тратит одну
единицу вместо двух (§6.7). При двух и трёх фотографиях так не выйдет: `sendMediaGroup`
клавиатуру не несёт — это ограничение Bot API, а не выбор.

У объединённой карточки оба `message_id` в `report` (§2.2) указывают на одно сообщение,
и правится она `editMessageCaption`: текста у сообщения с фотографией нет, есть подпись.

Два, а не три: `sendLocation` дал бы нативную точку, но ссылка на Яндекс Карты в подписи
открывает приложение с тем же результатом и не добавляет третьего уведомления группе
(US-027).
...
`message_id` карточки сохраняются в `report` (§2.2): по ним находится заявка, когда
волонтёр отвечает фотографиями (§6.8) или ссылкой на публикацию (§6.9). Поиск идёт
по обоим полям, поэтому объединённая карточка находится так же, как раздельная.

**Язык бота — узбекская латиница, одна на всё.** Группа волонтёров одна, переключателя
языка в ней нет; словарь лежит в `apps/api/src/telegram/labels.ts` и повторяет
формулировки статусов и причин из локали сайта (`docs/domain/glossary.md`), чтобы житель
и волонтёр читали об одной заявке одно и то же. Район и категория берутся из `name_uz`.

В статусе `IN_PROGRESS` карточка добавляет строку о том, что заявка закрывается
фотографией «после» **ответом на карточку** (§6.8), в `DONE` без ссылки — о том, что
ссылка на публикацию присылается так же (§6.9). Других статусов эти строки не касаются:
подсказка имеет смысл там, где действие возможно.
...
- **Причина из списка.** Нажатие → правка того же сообщения #2 (`editMessageText`,
  а у объединённой карточки `editMessageCaption` — §6.3): вместо кнопок статусов —
  кнопки причин и «Назад». Новых сообщений в группе не появляется.
...
залить вредоносный файл. Всё остальное — шум.
...
а целый район. При этом совсем без лимитов сервис кладётся одним скриптом.
...
- **CSP** (заголовок nginx, шаблон `docker/edge/security-headers.conf.template`):
  ...
  из переменной среды. Фотографии в `img-src` отдельного источника больше не требуют:
  они на нашем origin и покрыты `'self'` (ADR-0009).
...
`pg_dump` раз в сутки, gzip, во внешнее S3-совместимое хранилище под отдельным префиксом,
хранение 14 копий. Проверка восстановления — раз в квартал на dev-машине.

**Фотографии бэкапятся тоже** — с 19.08.2026, когда они переехали на диск сервера
(ADR-0009). Пока снимки лежали в бакете, реплики держал провайдер и копировать было
нечего; теперь единственная копия — диск прода, и его отказ означает безвозвратную потерю
фотографий «до»: яма уже отремонтирована, житель её не переснимет. Выгрузка
инкрементальная, по метке времени прошлой удачной синхронизации — имя файла есть sha256
его содержимого, значит выгруженное однажды не меняется никогда.

Из этого следует жёсткое требование к запуску: **`backup.sh` обязан отработать до анонса**.
До первого прогона копий фотографий не существует вовсе.
...
Загрузка подложки в E2E заглушается: скачивание 19 МБ архива тайлов делает тест
медленным и зависимым от сети, хотя проверяем мы не отрисовку. Проверяется, что
координаты попали в форму, а не что нарисовался тайл. Отрисовку карты и чистоту консоли
браузера проверяет ручной прогон в поднятом `docker compose` — оба дефекта карты
в 002 были невидимы и для типов, и для юнит-тестов.
...
Обязательный раздел: 1 ГБ на dev — жёсткое ограничение (`CLAUDE.md`), а не пожелание.
...
Прод переехал с 2 ГБ на 8 ГБ, но **лимиты контейнеров остались прежними**. Они выведены
из нагрузки, а не из размера машины: поднять потолок там, где до старого никто не дошёл,
значит потерять page cache Postgres и запас на пик sharp, ничего не выиграв. Пересмотр
лимитов — отдельное решение по измерению, а не следствие смены тарифа.

Обе среды поднимаются одним `docker-compose`, различаясь только файлом лимитов
и переменными.

### 12.1 Прод — Contabo Cloud VPS 4: 4 vCPU, 8 ГБ, 100 ГБ SSD
...
| ОС + dockerd | ~350 МБ | |
| Свободно | **~6,5 ГБ** | Page cache Postgres, пики sharp, `pg_dump` |
...
На 8 ГБ он превращается в чистую формальность, но стоит ноль и снимается только вместе
с доказательством, что не нужен.

**Узкое место сместилось с памяти на процессор.** vCPU у Contabo разделяются с соседями
по физической машине, и в пиковые часы часть тактов уходит им (CPU steal). Расчётный
запас это переживает: всплеск §1.6 требует 0,83 фото/с при пропускной способности sharp
5–8 фото/с. Проявится steal не отказом, а задержкой превью — очередь §5.4 растёт, заявки
принимаются как обычно (§5.7). Величина steal под нагрузкой меряется на проде вместе
с нагрузочной проверкой §11.5 [требует проверки].
...
2. делает хранение фотографий предметом отдельной проверки локализации
...
С марта 2026 довод стал жёстче, а не мягче. Закон ЗРУ-1125 сузил обязательную
локализацию ровно до биометрических и генетических данных (и данных абонентов связи).
Прод стоит в Германии, dev — в `us-central1`: сегодня это законно **именно потому**,
что биометрии в системе нет. В день, когда появится распознавание, вся инфраструктура
разом окажется вне закона — переносить придётся не фичу, а обе среды.
...
| --- | --- | --- | --- |
| 1 | ~~Условия использования Яндекс JS API v3 и тайлов~~ — **снято** ADR-0008: подложка своя, стороннего картографического API в системе нет | ADR-0008, §7.4 | — |
| 2 | ~~Наличие и API `YMapClusterer`~~ — **снято** ADR-0008: кластеризация выполняется средствами MapLibre по источнику GeoJSON с `cluster: true` | §3.4, §7.4 | — |
...
| 7 | ~~Архитектура прод-машины (amd64/arm64)~~ — **снято**: Contabo Cloud VPS 4 это x86 (AMD EPYC), prebuilt-бинари sharp есть, multi-arch сборка не нужна | §5.3, §12.1 | — |
| 8 | ЗРУ-547 в редакции ЗРУ-1125 от 26.03.2026: локализация сузилась до биометрии, генетики и данных абонентов связи — сверить основание ст. 27-1 ч. 3 для хранения за рубежом | §15, §9.8 | Нет; запрет биометрии действует и без сверки |
...
| 19 | Задержка и тарификация операций Cloupard S3 из сети прод-машины | §5.4, §5.7 | Число воркеров и стоимость |
...
| 23 | Фактический CPU steal на Contabo под нагрузкой | §12.1, §5.4 | Нет; запас шестикратный, растёт только задержка превью |
| 24 | ~~Задержка из Ташкента до прод-машины~~ — **снято** замером 19.08.2026 из сети AS8193 Uzbektelekom: Contabo EU 110 мс RTT против 196 мс у нынешнего dev в `us-central1` и 6 мс до ташкентских хостов; TTFB по HTTPS 352 мс, из них ~120 мс на TLS | §7.7, ADR-0008 | — |
