...
| --- | --- | --- |
| `db` | **320 МБ** | `shared_buffers=96MB`, `work_mem=2MB`, `max_connections=12` |
| `api` | **288 МБ** | `--max-old-space-size=192`, photo-воркер = 1, семафор приёма = **2** (4 × 4 МБ буферов), Prisma `connection_limit=5` |
| `edge` | **64 МБ** | `proxy_cache` `max_size=16m` |
| **Итого** | **672 МБ** | |
| ОС + dockerd | ~250 МБ | |
| Свободно | **~80 МБ** | |
