...
export interface ReportPhotoView {
  ...
  url: string
  previewUrl: string
  /** Размеры полного кадра в пикселях. Нужны просмотрщику: без них он не знает,
   *  какое место занять под снимок, и открывается рывком — сначала пустой экран,
   *  потом прыжок в размер загруженного изображения. В базе они уже лежат, считает
   *  их обработчик фотографий (SRS §5.4). */
  width: number
  height: number
}
