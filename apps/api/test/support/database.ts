 ...
 *
 *  База берётся из `TEST_DATABASE_URL`, по умолчанию — та же, что поднимает
 *  docker-compose для разработки. Отдельная переменная нужна, чтобы прогон тестов
 *  не вычищал базу, в которой разработчик держит свои заявки. */
