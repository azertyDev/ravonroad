 ...
 *
 *  Обе темы: тёмная объявлена полностью, и незамеченной она ломается легче светлой —
 *  на неё смотрят реже. */
const AA_TEXT = 4.5
...
interface Pair {
   ...
   *  существует только в одной: жёлтая подложка заголовка объявлена для светлой. */
  themes?: Theme[]
}
...
const PAIRS: Pair[] = [
  ...
  // Основная кнопка и жёлтые поля: счётчик кампании, полоса офлайна, шапка таблицы.
  // Наведение и нажатие меняют фон, а подпись остаётся той же — проверяются все три.
  { what: 'подпись основной кнопки', fg: '--text-on-accent', bg: '--accent', min: AA_TEXT },
  { what: 'подпись кнопки под курсором', fg: '--text-on-accent', bg: '--accent-hover', min: AA_TEXT },
  { what: 'подпись нажатой кнопки', fg: '--text-on-accent', bg: '--accent-press', min: AA_TEXT },

  // Жёлтый как текст: на светлой теме это затемнённый signal-700, на тёмной сам
  // signal-500. Один токен, два значения — и проверяется он в обеих темах.
  ...SURFACES.map((bg) => ({ what: `жёлтый как текст на ${bg}`, fg: '--text-accent', bg, min: AA_TEXT })),

  // Вторая строка заголовка главной: на светлой теме жёлтая подложка, на тёмной
  // жёлтый текст прямо на странице. Носитель приёма разный, требование одно.
  {
    what: 'обещание бригады на жёлтой подложке',
    fg: '--hero-accent-ink',
    bg: '--hero-accent-bg',
    min: AA_TEXT,
    themes: ['light'],
  ...
  {
    what: 'обещание бригады жёлтым на странице',
    fg: '--hero-accent-ink',
    bg: '--surface-page',
    min: AA_TEXT,
    themes: ['dark'],
  ...
  // Статусное поле страницы заявки: цвет один в обеих темах, поэтому текст на нём
  // выбран по контрасту каждой пары, а не назначен белым (colors.css).
  ...STATUSES.map((status) => ({
    what: `статус ${status}: текст на статусном поле`,
    fg: `--status-${status}-on-field`,
    bg: `--status-${status}-field`,
    min: AA_TEXT,
  })),
  ...
  // житель не видит, куда писать (WCAG 1.4.11). Декоративный --border-1 и рамка плашки
  // --border-2 сюда не входят: и карточку, и плашку опознают по содержимому.
  ...SURFACES.map((bg) => ({
    what: `граница поля ввода на ${bg}`,
    fg: '--border-field',
    bg,
    min: AA_LARGE,
  })),
  // Контур вторичной кнопки: заливки у неё нет, и границей она и опознаётся.
  ...SURFACES.map((bg) => ({
    what: `контур вторичной кнопки на ${bg}`,
    fg: '--border-control',
    bg,
    min: AA_LARGE,
  })),
  ...
  // Пары «кольцо на самой кнопке» здесь нет намеренно: --focus-outline-offset отодвигает
  // контур на 2 px наружу, и лежит он на поверхности вокруг элемента, а не на его заливке.
]
