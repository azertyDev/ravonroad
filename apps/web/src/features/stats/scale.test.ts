...
describe('campaignScale', () => {
  ...
  it('одометр набирается из цифр без разделителей', () => {
    // Разделитель разрядов выбрасывается: его рисует раскладка ячеек. Что сам
    // разделитель — тонкий U+2009, а не U+00A0, проверяет number.test.ts;
    // здесь проверяется только то, что в ячейки он не попал.
