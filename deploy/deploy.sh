...
# трёх слоёв одновременно на e2-micro с работающим стеком съедает свободные ~80 МБ
# (SRS §12.2). Порядок — migrate первым: он нужен раньше остальных.
pull_images() {
...
if health_ok; then
  ...
  [ -n "$PREVIOUS" ] && set_env PREVIOUS_IMAGE_TAG "$PREVIOUS"
