...
#
# Чистка по возрасту (`until=72h`) с этим не справляется, и это проверено: 19.08.2026
# за день набежало 62 образа и 24 ГБ, диск дошёл до 98% — все образы были моложе порога,
# поэтому не удалялось ничего. Порог по времени защищает от редких выкаток и бесполезен
# ровно в тот день, когда выкаток много.
#
# Поэтому оставляем не «свежие», а **нужные**: разворачиваемый тег и тот, что развёрнут
# сейчас — он же цель отката. Больше двух версий на машине не нужно никогда, всё
# остальное вернёт `pull_images` из GHCR, просто медленнее.
#
...
prune_images() {
  echo "==> чищу образы, кроме $TAG и ${PREVIOUS:-—}"
  # `case`, а не `[ … ] && continue`: AND-список с ложным условием возвращает 1, и `set -e`
  # убил бы подоболочку цикла на первом же образе, который надо оставить. Чистка молча
  # прекратилась бы, а диск продолжил заполняться — тот же класс ошибки, что описан выше
  # у записи PREVIOUS_IMAGE_TAG.
  ...
  # Слои, оставшиеся без тега после rmi, и кэш сборки — их держать незачем: docker build
  # на серверах запрещён (SRS §12.2 п.1), значит кэшу тут взяться неоткуда.
